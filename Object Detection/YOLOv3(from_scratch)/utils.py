...
def non_max_suppression(inputs, model_size, max_output_size, 
    ...
    bounding_box, confidence, class_probabilities = tf.split(inputs, [4, 1, -1], axis=-1)
    bounding_box = bounding_box/model_size[0]
    scores = confidence * class_probabilities
    boxes, scores, classes, valid_detections = tf.image.combined_non_max_suppression(
        boxes=tf.reshape(bounding_box, (tf.shape(bounding_box)[0], -1, 1, 4)),
